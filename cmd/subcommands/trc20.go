...
func usdtSub() []*cobra.Command {
	cmdSend := &cobra.Command{
		Use:     "send <ADDRESS_TO> <AMOUNT>",
		Short:   "send USDT tokens to an address",
		Args:    cobra.ExactArgs(2),
		PreRunE: validateAddress,
		RunE: func(cmd *cobra.Command, args []string) error {
			if signerAddress.String() == "" {
			...
			// get contract decimals if any
			tokenDecimals, err := conn.TRC20GetDecimals(contract)
			if err != nil {
				tokenDecimals = big.NewInt(0)
			}

			amount, _ := decimals.ApplyDecimals(value, tokenDecimals.Int64())
			tx, err := conn.TRC20Send(signerAddress.String(), addr.String(), contract, amount, feeLimit)
			if err != nil {
				return err
			}
	...
	cmdBalance := &cobra.Command{
		Use:     "balance <ADDRESS_TO> ",
		Short:   "get USDT balance from contract",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			value, err := conn.TRC20ContractBalance(addr.String(), contract)
			if err != nil {
				return err
			}
...
func init() {
	...
	cmdUSDT.AddCommand(usdtSub()...)
	RootCmd.AddCommand(cmdUSDT)
}
